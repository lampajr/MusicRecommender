...
def update_similarity_matrix_topk(data, k=100, verbose=True):
    """
    Iterates over each column and keep only the top-k similar items
    :param data: squared similarity matrix
    :param k: knn
    :param verbose: default True
    :return:
    """
    values, rows_indices, cols_indptr = [], [], []
    n_items = data.shape[0]

    # operates column-wise
    data_matrix = check_matrix(data, format='csc')

    start_time = time.time()
    for item_idx in range(n_items):
        cols_indptr.append(len(values))

        start_position = data_matrix.indptr[item_idx]
        end_position = data_matrix.indptr[item_idx + 1]

        column_data = data_matrix.data[start_position:end_position]
        column_row_index = data_matrix.indices[start_position:end_position]

        idx_sorted = np.argsort(column_data)  # sort by column
        top_k_idx = idx_sorted[-k:]

        values.extend(column_data[top_k_idx])
        rows_indices.extend(column_row_index[top_k_idx])

    cols_indptr.append(len(values))

    # During testing CSR is faster
    W_sparse = sp.csc_matrix((values, rows_indices, cols_indptr), shape=(n_items, n_items), dtype=np.float32)
    W_sparse = W_sparse.tocsr()
    W_sparse.eliminate_zeros()
    if verbose:
        print("Sparse KNN similarity matrix generated in {:.2f} seconds".format(time.time() - start_time))
...
class CosineSimilarity:
    ...
    def compute_similarity(self, start_col=None, end_col=None, block_size=100):
        ...
        sumOfSquared = np.array(self.dataMatrix.power(2).sum(axis=0), dtype=np.int32).ravel()
